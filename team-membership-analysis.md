...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ 控制器层（membership_controller.ex）                                    │
│  ┌──────────────────────────────────────────────────────────────────────────────┐   │
│  │ 职责 1: 源团队权限校验                                      │   │
│  │   - plug AuthorizeSiteAccess, [:owner, :admin]                            │   │
│  │   - 确保用户在源团队至少有 owner 或 admin 权限（站点访问权限）           │   │
│  │   - 确保用户能访问该站点（隐含权限校验）                                  │   │
│  └──────────────────────────────────────────────────────────────────────────────┘   │
│  │                                                                   │   │
│  ┌──────────────────────────────────────────────────────────────────────────────┐   │
...
│  ┌──────────────────────────────────────────────────────────────────────────────┐   │
│  │ 职责 2: 业务逻辑执行                                        │   │
│  │   - ensure_transfer_valid：不是转移到自身团队                               │   │
│  │   - ensure_can_take_ownership：目标配额校验（EE版本）                     │   │
│  │   - transfer_site：事务执行                                                 │   │
│  └──────────────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────────────┘
```
...
**职责边界总结:**
- **控制器层**：专注于**源团队权限**和**UI筛选**
  - 检查用户是否能访问源站点（隐含用户在源团队是 owner/admin）
  - 限制用户只能选择有权限的目标团队

- **服务层**：专注于**目标团队权限**和**业务逻辑**
  - 检查用户在目标团队的权限
  - 不检查源团队权限（由控制器层保证）
  - 执行站点转移的核心业务逻辑

### 6.2.2 绕过控制器直接调用服务的权限误判风险
...
1. **源团队权限绕开风险**
   - 服务层 `change_team/3` 只检查 `check_can_transfer_site(new_team, user)`
   - 不检查用户在源团队的权限
   - 如果绕过控制器直接调用，**任何知道目标团队的用户**（只要在目标团队是 owner/admin）都可以尝试转移站点
   - 例如：用户 A 在 Team B 是 admin，但在 Team A 没有权限。如果用户 A 绕过控制器直接调用 `Transfer.change_team(site_from_team_a, user_a, team_b)`，服务层不会检查用户 A 是否真的能访问 `site_from_team_a`

2. **目标团队选择绕开风险**
   - 控制器层通过 `teams(user, roles: [:owner, :admin])` 限制可选择的目标团队
   - 如果绕过控制器直接调用，可以传入任意 `team_id`
   - 只要服务层的 `check_can_transfer_site` 检查通过，就可能执行转移

3. **实际代码验证**

服务层代码（`transfer.ex:48-60`）:
```elixir
...
**风险结论:**
- `change_team` 链路采用**分层校验**设计：控制器层负责源团队，服务层负责目标团队
- 这种设计依赖**调用链完整性**，如果绕过控制器直接调用服务，可能导致**源团队权限被绕开**
- 建议：在服务层增加冗余的源团队权限检查，使服务层可以独立使用
...
`change_team` 链路采用分层校验：控制器层负责源团队权限校验，服务层负责目标团队权限校验。绕过控制器直调服务可能导致源团队权限被绕开，建议在服务层增加冗余的源团队权限检查，使服务层可独立使用。
